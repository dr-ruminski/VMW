...
window.InteractablePlane.prototype = {
  ...
  bindHighlight: function(){
    ...
    this.highlightMesh = new THREE.Mesh(
      new THREE.PlaneGeometry(this.mesh.geometry.parameters.width+0.010, this.mesh.geometry.parameters.height+0.010),
  ...
  watchXYIntersection: function(){
    ...
    var indexToBoneMeshIndex = [11,22,33,44, 11,22,33,44, 11,22,33,44, 11,22,33,44, 11,22,33,44];
    ...
    proximity.in( function(hand, intersectionPoint, key, index){
      ...
      if (index == 1 || index % 4 == 0 || index % 7 == 0 || index % 10 == 0 || index % 13 == 0) {
        httpGetAsync(arGlovesUrl, getPin(index), 500);
        voltage[1][index-1] = 0;
      } else if (index == 0 || index % 3 == 0 || index % 6 == 0 || index % 9 == 0 || index % 12 == 0) {
        httpGetAsync(arGlovesUrl, getPin(index), 700);
        voltage[1][index] = 1;
        voltage[0][index+1] = 0

      } else if (index % 2 == 0 || index % 5 == 0 || index % 8 == 0 || index % 11 == 0) {
        httpGetAsync(arGlovesUrl, getPin(index), 900);
        voltage[2][index] = 1;
        voltage[1][index+1] = 0;
      ...
      if(index == 1 || index == 4 || index == 7 || index == 10 || index == 13) {
        httpGetAsync(arGlovesUrl, getPin(index), 1024);
      }
      */  
    ...
    proximity.out( function(hand, intersectionPoint, key, index){
      ...
      if (index == 1
         || index % 4 == 0 || index % 7 == 0 || index % 10 == 0 || index % 13 == 0) {
        //httpGetAsync(arGlovesUrl, getPin(index), 700);
        if (voltage[1][index-1] == 1) {
          // do nothing
        } else {
          httpGetAsync(arGlovesUrl, getPin(index), 0);
...
//        console.assert(!isNaN(handPoint.z));
        ...
        console.log('distances['+key+'] = '+distance);
